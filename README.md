...
Каждая задача загружается через локальный роут `/api/puzzle`, который
обращается к публичному API Chess.com
`https://api.chess.com/pub/puzzle/random`. Сервер получает случайную
задачу и отдаёт её клиенту. Если запрос завершается ошибкой (например, из-за
